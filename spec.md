...
- 상태 전이 규칙(**한 회차 안에서**):
  ...
  - `FINISHED`에서 다른 상태로 되돌리면 `finished_at`을 비운다(아래 보류 항목 참조).
...
> **아직 정하지 않은 것** — 회차 모델로 바꾸면서 남은 두 질문이다([#50](https://github.com/irerin07/12books/issues/50) 3번).
>
> - 완독한 기록을 `READING`으로 되돌리는 것이 **상태 정정**인가 **새 회차 시작**인가. 지금은 같은
>   행의 완독일을 지운다 — 단일 상태 모델의 동작이 남은 것이다.
> - 완독 수를 세는 `countFinishedBetween()`이 `in_bookshelf = true`만 센다. 그래서 완독한 책을
>   서재에서 빼면 기록은 남아도 연간 목표에서 빠진다. 서재 전시와 독서 실적의 기준을 나눌지,
>   그리고 재독 두 회를 2권으로 셀지를 함께 정해야 한다.

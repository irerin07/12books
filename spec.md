...
| F2 | 책 검색 | 카카오 책 검색 API 프록시, **제목·저자 구분 검색**, 선택 시 내부 DB 업서트 |
...
소셜 로그인(OAuth2), 이미지 업로드(S3), 대댓글, 차단, DM,
독서 모임·챌린지, 추천 알고리즘, 검색 엔진(ES) 도입, 프론트엔드 클라이언트.

> **알림과 신고는 이 목록에서 나왔다.** 알림은 Phase 10으로, 신고·운영자 처리는 "정식 공개
> 전에 해야 하는 것"의 L1으로 들어가 이미 동작한다. 제외한다고 적어 둔 것을 뒤집은 자리라
> 지우지 않고 남긴다 — 무엇이 왜 올라왔는지는 `plan.md`에 있다.
...
- `readings`는 **서재에 꽂혀 있는 행이 (user, book) 당 1건**이다. 지난 독서는 행으로 남으므로
  같은 책에 여러 회차가 쌓인다(`V8__reading_sessions.sql`). 같은 책이 서재에 두 줄로 보이면
  진도를 어디에 적을지 알 수 없으므로, 유일해야 하는 것은 **지금 꽂혀 있는 것**뿐이다.
- 서재에서 빼는 것은 행 삭제가 아니라 `in_bookshelf` 변경이다. 담았다 뺐다 하는 것은 정상적인
  사용이고, 다시 담으면 진도가 그대로 남는다.
- 뺐던 책을 다시 담을 때 **이어서 읽을지 새로 시작할지는 사용자가 고른다**(`resume`).
  고르지 않고 보내면 `409 R003`으로 되묻는다 — 서버가 대신 고르면 지난 기록을 덮거나,
  고르지도 않은 "새로 시작"이 된다. `resume=true`는 그 행을 다시 꽂고, `false`는 새 회차를
  만든다.
- 상태 전이 규칙(**한 회차 안에서**):
  - 어떤 상태에서든 `READING`으로 전환 가능 → `started_at`이 비어 있으면 이때 채운다.
  - `FINISHED` 전환 시 `finished_at` 기록, `current_page`를 `page_count`로 맞춘다.
  - `FINISHED`에서 다른 상태로 되돌리는 요청은 **정정인지 재독인지 밝혀야 한다**(`reread`).
    밝히지 않으면 `409 R004`로 되묻는다 — 담기의 `resume`과 같은 모양이다.
    - `reread=false`(정정) — 같은 행을 고치고 `finished_at`을 비운다. 잘못 눌렀다는 뜻이다.
    - `reread=true`(재독) — **새 회차를 만든다.** 지난 완독은 완독인 채로 남고, 새 행이
      서재에 꽂힌다(꽂힌 행은 (사람, 책)당 하나다). 총 쪽수는 책의 속성이라 물려받고,
      진도와 별점은 물려받지 않는다.

  > 서버가 어느 쪽인지 짐작하면 반대쪽 사용자의 기록이 조용히 망가진다 — 정정으로 처리하면
  > 재독하는 사람의 지난 완독일이 지워지고, 재독으로 처리하면 오타를 고치려던 사람의 서재에
  > 빈 회차가 생긴다.
- `current_page`는 감소도 허용한다(되돌아가 읽기). 단 0 이상, `page_count` 이하.
- 상태·총 쪽수·진도를 한 요청에 함께 보내면 **최종 조합으로 판단한다.** 300쪽인 줄 알았던 책이
  200쪽이었고 다 읽었다는 요청(`FINISHED`, `page_count=200`, 진도 생략)은 `200/200`으로 정리된다.
  보내지 않은 값을 위 규칙으로 채우는 것이며, **보낸 값을 덮지는 않는다** — 200쪽짜리에 250쪽을
  명시하면 거부다.
- 연간 목표(`reading_goals`)는 사용자가 설정하지 않으면 **기본 12권**으로 간주한다.
- 그 해의 완독을 **두 숫자로** 답한다.
  - `finishedCount` — 다 읽은 **책 종수**. 달성률의 분자다. "올해 몇 권 읽었나"가 질문이므로
    같은 책을 두 번 읽어도 한 권이다.
  - `finishedSessionCount` — **완독 회차 수**. 두 번 읽으면 둘이다. 달성률에는 쓰지 않는다 —
    얇은 책을 반복해 숫자를 올릴 수 있다.
  - **서재 포함 여부를 보지 않는다.** 서재는 "지금 무엇을 꽂아 두었나"이고 실적은 "그 해에
    무엇을 다 읽었나"라 기준이 다르다. 다 읽고 책장에서 내렸다고 읽지 않은 것이 되지 않는다.
...
- **동일한 팔로우 상태를 기준으로** 홈과 팔로잉 피드의 게시글 집합은 겹치지 않는다. 홈에서
  팔로잉을 빼기 때문이다. 섞는 비율은 화면이 정한다.
- 각 요청은 독립적으로 조회하므로 **요청 사이에 팔로우 관계가 바뀌면 두 응답을 합친 결과에
  중복이나 누락이 생길 수 있다.** 두 요청 사이의 공통 스냅샷은 제공하지 않는다.
...
| 메서드 | 경로 | 설명 |
|---|---|---|
| GET | `/books/search?q=&page=&target=` | 카카오 프록시 (`target`: `ALL`·`TITLE`·`AUTHOR`) |

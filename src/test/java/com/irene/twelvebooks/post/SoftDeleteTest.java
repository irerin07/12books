...
class SoftDeleteTest extends AbstractIntegrationTest {
	...
	void softDeletesReadingAndRevivesOnReshelve() throws Exception {
		...
		assertThat(rows("readings", "id = " + readingId + " and deleted_at is not null")).isEqualTo(1);

		// uk(user_id, book_id)가 그대로라 다시 담기는 insert가 아니라 되살리기다. 409가 아니다.
		mockMvc.perform(post("/api/v1/readings").header("Authorization", bearer)
						.contentType(MediaType.APPLICATION_JSON)
						.content("{\"bookId\": %d, \"status\": \"WANT_TO_READ\"}".formatted(freshBookId)))
				.andExpect(status().isCreated())
				.andExpect(jsonPath("$.status").value("WANT_TO_READ"));

		// 행이 늘지 않는다 — 같은 행이 되살아난 것이다(setUp의 것까지 둘).

...
public class ReadingLinker {
	 ...
	 * 서재에서 빼고 커밋하면, <b>이미 지나간 회차에 글이 매달린다</b> — 붙일지 말지를 정할 때
	 * 본 상태가 저장할 때는 더 이상 참이 아니다. 잠가 두면 빼기가 호출자의 커밋을 기다리므로,
	 * 판단이 저장까지 유효하다. 진도 수정도 같은 잠금을 쓰므로 함께 직렬화된다.
	 *
	 * <p><b>외래 키 위반은 여기서 일어날 수 없다.</b> 서재에서 빼기는 행을 지우지 않고

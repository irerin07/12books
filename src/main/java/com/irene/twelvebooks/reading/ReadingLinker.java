...
public class ReadingLinker {
	 ...
	 *
	 * <p>서재에 있는 기록을 연결하는 경로에서는 잠금 조회로 그 행의 서재 포함 여부를 확인한다.
	 * 잠금을 얻으면 글쓰기 트랜잭션이 끝날 때까지 같은 행의 서재 제외·수정이 대기한다. 먼저
	 * 서재에서 제외됐다면 이 경로는 연결 없이 글을 저장한다. <b>같은 기록에 대한 작업 순서를
	 * 정하는 것이지, 외래 키 위반을 막는 잠금이 아니다.</b>
	 *
	 * <p>외래 키 위반은 여기서 일어날 수 없다. 서재에서 빼기는 행을 지우지 않고
	 * {@code in_bookshelf}만 내린다({@code unshelve}) — {@code readings}에 하드 삭제 경로가 없다.
	 * 이 자리에 "빼면 FK 검사에 걸려 500이 된다"고 적혀 있었는데, 그건 빼기가 삭제이던
	 * 시절(V8 이전)의 설명이다.
	 *
	 * <p><b>지난 회차를 다시 담는 경로는 성격이 다르다.</b> 거기서는 {@code findAnyByIdForUpdate}가
	 * 서재 포함 여부와 무관하게 행을 잠그고, {@link #shelveAgainIfRemoved}가 <b>그 행을 실제로
	 * 고친다</b>(다시 꽂고 상태를 옮긴다). 읽고-고치고-쓰는 사이를 직렬화해야 하므로 잠금이
	 * 필요하고, 위의 순서 보장과는 다른 이유다.
	 *
	 * <p>빈 값은 "붙일 기록이 없다"는 뜻이다. 잠그기 전에 이미 서재에서 제외된 경우인데,
	 * 되살리지 않는 것은 방금 사용자가 뺀 책을 서버가 도로 담는 셈이 되기 때문이다. 서재에서
	 * 뺀 뒤에도 기존 글의 연결은 그대로 남으므로, 연결 없는 글도 정상 상태다.
	 *

...
public class ReportAdminService {
	...
	private void hide(Report report) {
		switch (report.getTargetType()) {
			...
			case COMMENT -> {
				...
				if (commentRepository.hide(report.getTargetId(), LocalDateTime.now(clock)) == 1) {
					commentRepository.findPostId(report.getTargetId())
							.ifPresent(postRepository::decreaseCommentCount);
				}
			}
			case USER -> {
	...
	private void restore(Report report) {
		switch (report.getTargetType()) {
			...
			case COMMENT -> {
				if (commentRepository.unhide(report.getTargetId()) == 1) {
					commentRepository.findPostId(report.getTargetId())
							.ifPresent(postRepository::increaseCommentCount);
				}
			}
			case USER -> {
			}

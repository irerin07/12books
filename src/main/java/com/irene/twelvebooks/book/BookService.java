...
public class BookService {
	...
	/**
	 * ISBN이 없는 책의 대체 유일 키. 카카오가 같은 책을 같은 문자열로 주는 동안에만 같은 키가
	 * 나온다.
	 *
	 * <p><b>근사 식별이다.</b> 같은 책의 표기가 바뀌면(부제 추가, 저자 표기 변경) 다른 책이
	 * 되고, 제목·저자·출판사가 같은 다른 판본은 한 책이 된다. 해시를 바꿔도 이 문제는
	 * 그대로다 — 해시는 문자열이 같은지를 볼 뿐 같은 책인지를 모른다. ISBN이 있는 책이
	 * 그쪽을 쓰는 이유다.
	 *

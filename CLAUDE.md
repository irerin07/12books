...
2. **접기로 한 축이 있으면 접었다고 말한다.** "발송자만 정해 주세요 — 전송은 SMTP로
   잡겠습니다"처럼 한 줄. 혼자 정할 수 있는 축이라도, 정하지 않은 채 기본값을 물려받는 일을
   막는다.
3. **그 기본값의 근거가 이 코드베이스인가.** 교체 비용·표준 API·새로 생기는 개념 수로 따진다.

`.claude/hooks/guard-question.ps1`이 기계적으로 잡히는 둘(분류 혼합·선택지 겹침)을 보고
**한 번만** 되돌려 보낸다. 같은 질문을 다시 보내면 통과한다 — 강제하려는 것은 특정 답이
아니라 한 번의 재검토이고, 막기만 하면 오탐이 교착이 된다. 판정 로직은
`.claude/hooks/guard-question.tests.ps1`이 지킨다.

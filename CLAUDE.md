...
- `premise-harness.md` — 만들거나 설명하기 전에 돌리는 검사. "이건 이럴 것이다"가 문장에 들어갈 때 연다.
...
전제는 주장처럼 생기지 않아서 검사를 빠져나간다. "지금은 이렇게 동작하니까"는 근거를 요구하는
문장이 아니라 **배경 설명처럼** 들린다. 그래서 출처 검사에도 선택지 검사에도 걸리지 않고, 그
위에 코드가 얹히고 질문이 만들어진 뒤에야 드러난다.

실제로 있었던 일 — 이슈가 "자동 담기의 필요성을 확인한다"고 적은 것을 보고 그것이 **아직 열린
결정이라고 전제한 채** 선택지 셋을 만들어 물었다. `spec.md` §4.4에 "책 담기를 잊어도 글은
써진다"가 요구사항으로 적혀 있었다. `decision-harness.md`는 열었지만 `spec.md`는 열지 않았고,
그래서 하네스의 "이미 정해져 있다" 칸을 **추론으로** 비워 뒀다.

그래서 셋을 본다.

1. **내가 무엇을 깔았는지 꺼낸다.** "당연히 이럴 것이다"에 해당하는 절을 문장에서 분리한다.
2. **그것을 지배하는 자리를 연다.** 사용자에게 보이는 동작은 `spec.md`, 기술 규약은 `plan.md`와
   이 파일, 작업의 범위·결정권은 **이슈 본문**, 지금 하는 일은 코드와 테스트다. **위에서부터**
   연다 — 코드가 그렇다는 것은 그게 요구사항이라는 뜻도 아니라는 뜻도 아니다.
3. **줄을 댈 수 있는지 본다.** 못 대면 아직 안 연 것이다. 찾아봤는데 없으면 그것도 결과다 —
   그때 비로소 열린 질문이고 `decision-harness.md`로 넘어간다.

**선택을 먼저 하고 근거를 나중에 찾지 않는다.** 그러면 근거가 선택을 바꿀 수 있었는지 스스로
알 수 없다. 탈퇴 계정의 프로필 수정을 막을 때 "글·댓글이 401이니 여기도 401"로 갔는데, 기준은
그게 아니라 "하드 삭제였다면 무엇이 나왔을까"(404)였다 — 401은 행이 남아 있음을 누설한다.

절차와 통과 못 하는 신호, 사례는 **`premise-harness.md`**에 있다. `source-harness.md`가 *남의*
문서를 봤다고 말하려면 실제로 봤어야 한다는 것이라면, 이쪽은 그 안쪽이다 — **우리 문서를 안
보고 우리 동작을 말하지 않는다.**
...
**잠금 순서** — 여러 테이블을 만지는 경로는 **같은 순서로** 잡는다. 반응은 `posts`를 먼저
잠그고(plan.md T5), 댓글 INSERT는 외래 키 때문에 `users`에 공유 잠금을 잡는다 —
`posts → users`다. 사용자 상태를 바꾸는 경로가 `users → posts`로 가면 그 둘이 서로를
기다린다.

> 순서를 맞출 수 없으면 **한 트랜잭션이 둘을 함께 쥐지 않게** 쪼갠다. 탈퇴가 그렇게 한다 —
> 표시(`users`)와 댓글 수 조정(`posts`)이 각자 자기 트랜잭션에서 끝난다. 대가는 중간에
> 죽으면 숫자가 남는다는 것인데, 나중에 다시 셀 수 있는 어긋남이라 교착으로 500을 주는
> 것과 바꿀 만하다.
...
**탈퇴도 예외가 아니다.** 계획서는 하드 삭제를 권고했지만 그렇게 하지 않기로 했다
(`V13__withdrawal.sql`). 계정에 `deleted_at`을 세우고 그 사람과 관련된 것이 조회에서 빠지게
한다. 보관은 1~3년이고, 그 뒤의 실제 파기와 개인정보 암호화는 별도 작업으로 남겨 두었다.

> 그래서 **유니크 제약이 살아 있는 계정에만 걸려야 한다.** 행이 남으면 이메일과 handle을
> 붙잡아 같은 주소로 다시 가입할 수 없고, 그러면 탈퇴가 영구 추방이 된다. 생성 컬럼으로
> 부분 유니크를 만든다(`active_email`·`active_handle`).

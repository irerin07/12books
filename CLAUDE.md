...
- `plan.md` — Phase 0~10 개발 로드맵. 기술 기반(T1~T7)과 공통 규약. **무엇을 만드는가**만 적는다.
- `tasks.md` — **어디까지 됐는가.** 진행 상황 표와 단계별 체크박스, 그리고 공개 전 목록(L1~L3).
- `decision-harness.md` — 무엇을 고르기 전에 돌리는 검사. 선택지를 내놓기 전에 연다.
- `source-harness.md` — 근거를 말하기 전에 돌리는 검사. 출처를 붙이려 할 때 연다.

지금 무엇을 만들지 모르겠으면 `tasks.md`의 **진행 상황** 표와 그 아래 체크박스를 본다 — 무엇이
끝났는지는 거기에만 있다(`plan.md`의 Phase 표에는 상태 칸이 없다. 무엇을 만드는가만 적는다).
**번호 순서가 곧 순서는 아니다** — 공개가 가까우면 "정식 공개 전에 해야 하는 것"이 먼저다.
...
1. **애초에 물을 것인가.** 그 축을 이미 정해 둔 문서를 **연다** — 사용자에게 보이는 동작은
   `spec.md`, 규약은 `plan.md`와 이 파일, 작업의 범위·결정권은 **이슈 본문**, 지금 코드가 하는
   일은 코드와 테스트다. 위에서부터 연다. 이미 정해져 있으면 묻지 않고 **인용해서 말한다.**
   결정권을 적을 때는 축마다 **인용**이나 **"확인 — 해당 없음"**을 붙인다. 그 줄이 없으면 읽는
   사람이 맞는지 틀린지 판정할 수 없고, 판정할 수 없는 줄은 적지 않은 것과 같다.
2. **이 선택지들이 같은 종류인가.** 다른 축이 섞이면 한 축이 다른 축의 성질처럼 보인다.
   축 사이에 제약이 있으면(한쪽 선택이 다른 쪽 후보를 줄이면) 먼저 밝힌다.
3. **접기로 한 축이 있으면 접었다고 말한다.** "발송자만 정해 주세요 — 전송은 SMTP로
   제안합니다"처럼 한 줄. 혼자 정할 수 있는 축이라도, 정하지 않은 채 기본값을 물려받는 일을
   막는다. 미룬 축은 **왜 미뤘는지와 무엇을 보면 다시 정하는지**를 함께 남긴다 — 조건 없는
   보류는 보류가 아니라 망각이다.
4. **그 기본값의 근거가 이 코드베이스인가.** 교체 비용·표준 API·새로 생기는 개념 수로 따진다.
...
>
> **파일을 여는 것으로도 보장되지 않는다.** 2026-09-18에 `decision-harness.md`를 열고 절차대로
> 축을 적은 다음 틀렸다 — 결정권 칸을 `spec.md`를 열지 않고 채웠기 때문이다. 그래서 이 하네스가
> 실제로 하는 일은 나를 막는 것이 아니라 **내가 건너뛴 것을 눈에 보이게 만드는 것**이고,
> 강제는 그다음에 리뷰가 한다. 위 1번의 인용 줄이 그 보이게 만드는 자리다.
...
> **다만 비밀번호 해시는 탈퇴 시점에 비운다**(`V15`). 보존 목적이 컬럼마다 다르다 — 콘텐츠를
> 잇는 `id`와 운영 기록에 필요한 `handle`·`display_name`은 남을 이유가 있지만, 해시는 탈퇴
> 계정에서 **할 수 있는 일이 없다.** 얻는 것 없이 유출 위험만 진다.

...
**삭제** — **행을 지우지 않는다.** 사용자에게 "삭제"인 동작은 전부 `deleted_at` 플래그를 세우고,
조회는 `deleted_at is null`로 좁힌다. 법적 요구(개인정보 파기 등)나 그에 준하는 별도 사유가 있을
때만 실제 삭제를 검토하고, 그때도 먼저 사유를 적는다.
...
유니크 제약이 걸린 테이블은 **되살리기 경로**가 필요하다. 지운 행이 키를 붙들고 있어 같은 값을
다시 넣으면 제약에 걸린다(`readings`의 `uk(user_id, book_id)`가 그렇다). 제약을 푸는 대신
"다시 담기 = 되살리기"로 다룬다 — 제약의 본뜻은 지운 뒤에도 옳다. MySQL에서 `deleted_at`을
유니크 키에 넣는 방법은 통하지 않는다: 유니크 인덱스가 NULL을 서로 다른 값으로 보기 때문에

...
**이 리포 안의 것도 같다.** "`spec.md`에 이렇게 적혀 있다", "코드에 그 경로가 없다"도 열지 않고
말하면 똑같이 가짜 신호다. 특히 **부재는 전수로만 확인된다** — 몇 군데 보고 "안 쓰인다"로 넘어가지
않는다. 실측이 뒷받침하는 것과 문서로 확인한 것도 구분한다. "동작이 그랬다"는 "메커니즘이
그것이다"가 아니다.

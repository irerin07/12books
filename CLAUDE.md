...
`railway.json`이 빌더와 헬스체크를 고정한다. 헬스체크 경로(`/actuator/health`)를 거는 이유는
Flyway가 V1부터 까는 동안 트래픽이 붙지 않게 하기 위해서다 — 안 걸면 첫 배포의 초기 요청들이
"아직 안 뜬 앱"을 만난다. UI에서 설정하지 않고 파일에 두는 것은 환경을 다시 만들 때 같은 일을
또 하지 않기 위해서다.

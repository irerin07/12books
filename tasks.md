...
- [x] **탈퇴하면 비밀번호 해시를 비운다**(`V15`). 보존 목적을 컬럼별로 갈랐다 — 콘텐츠를 잇는
      `id`와 운영 기록에 필요한 `handle`·`display_name`은 남을 이유가 있지만, **해시는 쓸 일이
      없다**(로그인도 재발급도 막혀 있다). 얻는 것 없이 유출 위험만 지는 값이라 상태 전환과
      같은 UPDATE에서 비운다. `email`은 건드리지 않았다 — 재가입 확인·분쟁 대응에 쓸 여지가
      있어 별도 판단이 필요하다. 다시 볼 시점 = 개인정보 파기 요건을 법률로 확인할 때
...
- [x] `V16__blocks.sql` — `uk(blocker_id, blocked_id)` + 자기 차단 금지 CHECK.
      행은 한 방향(누가 눌렀나), 보이지 않는 것은 **양방향**이다
- [x] **내용**을 가린다 — 글 다섯 화면은 `PostReadRepository.SELECT` **한 자리**에서 막힌다
      (상수를 공유하므로 새 목록을 추가해도 조건이 따라온다). 그 밖에 댓글 목록, 알림 목록과
      안 읽은 수
- [x] **관계는 가리지 않는다.** 내가 차단해도 상대가 나를 언팔한 것은 아니므로 팔로워·팔로잉
      목록에 그대로 있고, 팔로워 수는 모두에게 같은 값이다. 보는 사람마다 다른 수를 주면
      "A의 팔로워 수"가 A의 속성이 아니라 (A, 보는 사람)의 함수가 된다
- [x] **읽기 경로는 404**(`BlockGuard`). 프로필·글 목록·서재·팔로워/팔로잉 목록·사람 신고.
      목록 안의 사람만 거르고 프로필을 열어 두면 반쪽이 된다
- [x] **팔로우·언팔로우는 막지 않는다.** 자기 관계를 바꾸는 쓰기는 본인 몫이고, 걸어도 내용은
      그대로 가려진다. 막으면 상대가 실수로 언팔한 순간 차단이 풀릴 때까지 돌아올 수 없다
- [x] 팔로우는 **차단한 쪽만** 끊는다. 상대가 건 것은 남고 조회에서만 가려진다 — 차단은
      내 의사이지 상대의 의사를 지울 근거가 아니다
- [x] **규약 테스트**(`BlockConventionTest`) — 목록 쿼리가 조건을 빠뜨리거나 handle 경로가
      가드를 안 지나면 빌드가 빨개진다. 소프트 삭제 때 못 만든 하네스를 여기서 만들었다
- [ ] Phase 7의 사람 검색에도 조건을 단다 — 그 Phase가 아직 없다

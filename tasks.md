...
- [ ] Redis 고정 윈도 카운터 (새 인프라 없이)
- [ ] 대상: 로그인 · 가입 · 감상평 · 댓글 · 좋아요 · 팔로우
- [ ] `server.forward-headers-strategy` — 안 켜면 프록시 뒤에서 **전 사용자가 한 버킷**을 쓴다
- [ ] 429 + `Retry-After`, `ErrorCode` 추가
- [ ] 로그인 실패는 IP 기준. **이메일 단위 잠금은 계정 열거 통로다**
...
- [ ] **"행을 지우지 않는다" 규약의 첫 예외.** `CLAUDE.md`에 사유를 먼저 적는다
- [ ] 삭제 범위 결정 (권고: 전부 하드 삭제)
- [ ] FK가 RESTRICT라 **자식부터** 지운다. 소프트 삭제된 행도 함께
- [ ] `DELETE /me` + 비밀번호 재확인
- [ ] 같은 이메일로 재가입 가능한지 확인

...
- [x] **자격증명 번호**(`users.credential_version`, `V12`) — 무효화만으로는 부족하다.
      옛 비밀번호로 이미 검증을 통과한 로그인이 무효화 <b>뒤에</b> 만든 세션은 끊긴 적이
      없어 계속 재발급된다. 번호를 비밀번호와 <b>같은 행</b>에 둬 함께 읽고 함께 바꾼다 —
      따로 두면 옛 해시에 새 번호가 붙거나, 번호가 먼저 만료돼 멀쩡한 세션이 끊긴다

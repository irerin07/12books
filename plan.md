...
- `V9__reports.sql` — `reports(reporter_id, target_type, target_id, reason, status)` +
...
- `V10__blocks.sql` — `blocks(blocker_id, blocked_id)` + `uk` + 자기 차단 금지 CHECK
...
spec.md §3.2의 제외 항목(소셜 로그인, 알림, 이미지 업로드, 대댓글, DM, 독서 모임,

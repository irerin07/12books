...
탈퇴도 같다. 행위자가 탈퇴하면 그가 만든 알림을 어떻게 할지 L2-1의 삭제 범위에 포함한다.
...
### L2-1. 회원탈퇴 · Backend · Policy

**이 프로젝트의 "행을 지우지 않는다" 규약이 처음으로 예외를 발동하는 자리다.**
`CLAUDE.md`가 그 예외를 이미 적어 뒀다 — 법적 요구가 있을 때만, 사유를 먼저 적고.

정할 것은 삭제 범위다. 권고는 **전부 하드 삭제**다(익명화보다 구현도 정책도 단순하다).
다만 순서가 문제다.

- `posts.author_id`와 `comments.author_id`가 RESTRICT라 **자식부터** 지워야 한다
- 지금 구조가 섞여 있다 — `follows`·`post_likes`는 하드 삭제, `posts`·`comments`는 소프트
  삭제다. 소프트 삭제된 행도 함께 지워야 하므로 조회 조건을 빼고 지우는 경로가 따로 필요하다
- `DELETE /me` + 비밀번호 재확인
...
| L2 | 회원탈퇴 · 차단 · 약관과 정책 · 이메일 인증 | `V10__blocks` 외 |
| L3 | 감상평 수정 · 기록 플로우 · 도메인 · 카카오 확인 | — |

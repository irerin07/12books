...
- **피드에 본인 글은 넣지 않는다.** 처음에는 "자기 글이 안 보이는 타임라인은 어색하다"고 보아
  포함했는데, 화면을 만들어 보니 반대였다. 홈에 내 글과 남의 글이 섞이면 무엇을 보는 화면인지
  흐려진다 — 인스타·트위터가 홈에 자기 글을 섞지 않는 것과 같다.
  내 글은 `GET /users/{handle}/posts`로 따로 본다.
  아무도 팔로우하지 않으면 타임라인은 빈다. 그 빈 자리가 곧 "팔로우할 이유"이고,
  화면은 그때 둘러보기를 권하면 된다.
...
**완료 기준**
A가 B를 팔로우 → A의 `/feed`에 B의 글만 보이고 A 자신의 글도 C의 글도 안 보임 →
A의 글은 `/users/A/posts`에 보임 →

...
**목표**: 소셜 루프를 닫는다. 반응이 있었는데 본인이 모르면 반응이 없는 것과 같다.

`spec.md` §3.2가 MVP 밖으로 뺐던 항목인데, 좋아요·댓글이 들어온 뒤 다시 필요해졌다.

**산출물**
- `V9__notifications.sql`
- `notification/Notification` — 종류, 행위자, 대상, 읽은 시각
- `GET /notifications?cursor=` · `GET /notifications/unread-count`
- `PATCH /notifications/{id}/read` · `POST /notifications/read-all`

**알림이 생기는 일** — 좋아요 · 댓글 · 팔로우. 팔로우를 넣는 이유는 테이블과 화면이 같고,
**이것이 있어야 맞팔이 돈다**.

**기술 상세**

- **생성은 커밋 이후에 한다.** 좋아요 트랜잭션 안에서 알림을 만들면 알림 저장이 실패했을 때
  좋아요까지 롤백된다. 부가 기능이 본 기능을 되돌리면 안 된다.
  `@TransactionalEventListener(AFTER_COMMIT)` + 독립 트랜잭션.
- **안 읽은 개수에 카운터를 두지 않는다.** `users`에 카운터를 더하면 Phase 6에서 세 번 겪은
  잠금 승격 교착이 그대로 온다 — 자식 insert가 부모에 잡는 공유 잠금과 카운터 UPDATE의
  배타 잠금이 물린다. 팔로워 수와 같이 `count` 쿼리로 시작한다.
- **같은 일은 한 건만.** `uk(recipient_id, type, actor_id, target_type, target_id)`.
  하트를 껐다 켰다 해도 알림이 쌓이지 않는다. 제약 위반은 오류가 아니라 "이미 알렸다"이므로
  삼킨다. **취소해도 알림은 남긴다** — 알림은 "이런 일이 있었다"는 기록이고 취소가 그 사실을
  없애지는 않는다.
- **본인 행동은 알리지 않는다.** 내 글에 내가 좋아요·댓글.
- **행위자와 글은 페이지 전체를 모아 한 번씩 읽는다.** 알림 20건에 쿼리 20번이 나가면 안 된다
  — `PostService.assemble`과 같은 방식.
- 읽음은 `read_at`. 목록은 `id desc` 커서.

**실시간은 하지 않는다.** 화면 진입 시 조회와 주기적 갱신으로 간다. WebSocket·SSE는 실제
요구가 관측된 뒤에.

**나중에 할 것 — 묶기**

"A님 외 3명이 좋아합니다" 같은 집계는 **의도적으로 미룬다**. 묶으면 읽음 처리가 "어디까지
읽었나"로 복잡해지고, 지금 트래픽에서는 묶일 만큼 쌓이지도 않는다. 넣게 되면 알림 행을
합치지 말고 **조회 시점에 묶는** 쪽을 먼저 검토한다 — 합친 행은 되돌릴 수 없다.

**차단·탈퇴와 엮인다**

이 Phase를 "정식 공개 전에 해야 하는 것" L2보다 **먼저** 둔다. 차단이 들어올 때
"차단한 사람의 알림은 오지 않는다"를 함께 지켜야 하는데, 알림이 이미 있으면 차단 작업에서
한 번에 처리된다. 순서가 반대면 차단을 끝낸 뒤 알림에서 다시 뚫린다.

탈퇴도 같다 — 실제로 L2-1에서 함께 처리했다. **알림은 남기고 행위자 정보만 생략한다**
(`NotificationResponse.of`). 알림은 "이런 일이 있었다"는 기록이고, 탈퇴가 그 일을
없던 것으로 만들지는 않는다. 다만 탈퇴한 프로필을 노출하지는 않는다.

**완료 기준**
남이 내 글에 좋아요·댓글을 남기거나 나를 팔로우하면 알림이 하나 생긴다 →
내가 한 행동은 알림을 만들지 않는다 → 같은 사람이 같은 대상에 반복해도 한 건이다 →
안 읽은 개수가 맞고, 읽음 처리하면 0이 된다 → 목록 크기를 바꿔도 쿼리 수가 늘지 않는다.

---

# 정식 공개 전에 해야 하는 것

Phase 9까지는 **제품을 만드는** 계획이다. 이 절은 **불특정 다수에게 열기 위한** 조건이고,
2026-09-13 출시 준비 리뷰에서 나왔다. 판정은 이랬다.

> 클로즈드 베타는 GO. 불특정 다수 공개는 NO-GO.

막는 것이 UI가 아니라 **계정 복구·탈퇴·신고·운영**이라는 점이 핵심이다. 화면 다듬기는
이 목록이 끝난 뒤에 해도 늦지 않다.

**이 절은 `spec.md` §3.2의 제외 항목을 일부 되돌린다.** 거기서 "차단·신고"를 MVP 밖으로
빼 뒀는데, UGC 서비스를 공개하는 조건에 들어가므로 범위 안으로 들어온다.

## L1 — 이게 없으면 클로즈드 베타도 위험하다

### L1-1. 요청 제한 · Backend

**지금 완전히 무방비다.** 나머지 항목은 사람이 대응할 시간이 있지만 이것만은 없다.
누군가 스크립트로 가입·글쓰기를 돌리면 막을 수단이 하나도 없다.

- 대상: 로그인, 가입, 감상평·댓글 작성, 좋아요·팔로우
- Redis가 이미 refresh 세션을 들고 있으므로 **새 인프라 없이** 고정 윈도 카운터로 시작한다
- 키는 인증 전이면 IP, 인증 후면 userId. Railway 프록시 뒤라 원 IP를 얻으려면
  `server.forward-headers-strategy`를 켜야 한다 — 안 켜면 모든 요청이 프록시 한 IP로 보여
  **전체 사용자가 한 버킷을 공유한다**
- 응답은 429 + `Retry-After`. `ErrorCode`에 항목을 더한다

> 로그인 실패를 **이메일 단위로** 잠그면 계정 열거 통로가 된다 — "이 이메일은 잠겼다"가
> 곧 "이 계정이 있다"다. IP 기준을 1차로 두고, 이메일 단위는 잠금이 아니라 지연으로 다룬다.

**완료 기준** — 한도 초과 요청이 429를 받고, 정상 사용 빈도는 걸리지 않는다. 프록시 뒤에서
서로 다른 클라이언트가 서로 다른 버킷을 쓴다.

### L1-2. 비밀번호 재설정 · Backend · Frontend · Infra

비밀번호를 잊으면 운영자에게 연락하는 수밖에 없는 상태다.

- `POST /auth/password-reset` — 이메일을 받고 **언제나 204**. 계정 유무를 알려주지 않는다
- `POST /auth/password-reset/confirm` — 토큰과 새 비밀번호
- 토큰은 Redis에 **해시로** 저장, TTL 30분, 1회용. 성공하면 그 사용자의 refresh 세션을
  전부 끊는다 — 비밀번호를 바꾸는 이유가 보통 탈취이기 때문이다
- **메일 발송자가 새 외부 의존으로 들어온다.** 외부 API 규약이 그대로 적용된다 —
  실제로 보내 보고 `external-apis.md`에 실측을 남긴다

**완료 기준** — 링크 재사용 401, 만료 후 401, 재설정 뒤 기존 기기의 재발급이 끊긴다.

### L1-3. 신고와 관리자 처리 · Backend · Policy

신고 버튼만 있고 쌓이는 곳이 없으면 신고 기능을 만든 것이 아니다.

- 신고 테이블 — `reports(reporter_id, target_type, target_id, reason, status)` +
  `uk(reporter_id, target_type, target_id)`로 같은 대상 중복 신고를 막는다.
  `target_type`은 POST · COMMENT · USER
- `POST /posts/{id}/reports`, `/comments/{id}/reports`, `/users/{handle}/reports`
- 관리자: `GET /admin/reports?status=PENDING`, `PATCH /admin/reports/{id}`
- 권한 구분이 없으므로 `users`에 역할 컬럼을 더한다

> **운영자 숨김은 작성자 삭제와 다른 사건이다.** `deleted_at`을 재활용하지 말고 별도
> 컬럼을 둔다. 둘을 합치면 "작성자가 지운 글"과 "운영자가 내린 글"을 구분할 수 없고,
> 신고가 기각됐을 때 되돌릴 수도 없다.

**기각과 공개는 한 요청에서 갈린다** — `PATCH /admin/reports/{id}`의 `restore`

"이 신고의 주장은 타당하지 않다"와 "이 글을 다시 공개한다"는 다른 판단이다. 신고가 여럿
달린 글에서 하나를 기각하는 것은 흔한 일이고, 그때마다 글이 열리면 의도하지 않은 공개가
된다. 그렇다고 판단만 저장하고 공개를 따로 묻는 식이면 "기각했는데 글이 그대로네"를 뒤늦게
발견한다. 그래서 한 요청으로 받되 **서버가 짐작하지 않는다.**

- `restore`를 보내지 않으면 `409 S004`로 되묻는다
- `restore=false` — 판단만 남기고 대상은 내려 둔 채다
- `restore=true` — 다른 인정된 신고가 없으면 공개한다
- `restore=true`인데 **다른 인정된 신고가 남아 있으면 `409 S005`로 거절한다.** 조용히
  넘기면 운영자는 눌렀는데 아무 일도 일어나지 않은 화면을 본다. 모르는 사실을 알려 주고
  그것부터 처리하게 한다
- 인정(`ACTIONED`)과 사람 신고에는 묻지 않는다 — 전자는 언제나 내리는 것이고, 후자는 내린
  콘텐츠가 없다

> 남은 인정 신고는 **잠금 읽기**로 본다. 평범한 조회는 트랜잭션 시작 시점의 스냅숏을 보므로
> 방금 다른 운영자가 인정한 신고를 놓친다. 대상 행을 **먼저** 잠근 뒤에 읽는다 — 순서가
> `reports → posts`로 뒤집히면 교착이다(실제로 한 번 겪었다).

**완료 기준** — 신고가 목록에 뜨고, 숨김 처리하면 피드·책별·사람별·단건 조회에서 모두
사라지며, 공개를 고르면 다시 보인다.

## L2 — 공개 직전

### L2-1. 회원탈퇴 · Backend · Policy — **완료** ([#49](https://github.com/irerin07/12books/pull/49))

> **여기 적혀 있던 권고("전부 하드 삭제")를 따르지 않았다.** 아래는 그때의 계획이 아니라
> 실제로 내린 결정이다. 계획이 뒤집힌 자리라 무엇을 왜 바꿨는지 남긴다.

**"행을 지우지 않는다" 규약의 예외를 발동하지 않기로 했다.** 계획은 하드 삭제를
권고했지만(익명화보다 단순하다는 이유), 지운 뒤에 오는 질문에 답할 방법이 없다 —
"잘못 지웠으니 되살려 달라", "이 사람이 뭘 썼다가 지웠나", "지난달 통계가 왜 달라졌나".
탈퇴도 `deleted_at`으로 처리한다. 보관은 1~3년이다.

- `V13__withdrawal.sql` — `users.deleted_at` + **부분 유니크**(생성 컬럼 `active_email`
  ·`active_handle`). 행이 남으면 유니크가 이메일과 handle을 붙잡아 **탈퇴가 영구 추방**이
  된다. 살아 있는 계정에만 걸리는 제약이 필요하다
- 조회가 탈퇴자를 거른다 — 글·댓글·프로필·팔로워 목록·알림의 행위자까지
- `hidden_at`을 재활용하지 않는다. 합치면 신고 기각으로 되돌릴 때 탈퇴자의 글까지 살아난다
- `DELETE /me` + 비밀번호 재확인. 되돌릴 수 없는 일이라 토큰만으로 받지 않는다
- 세션 전부 무효화. 남은 access 토큰으로도 쓰지 못한다([#54](https://github.com/irerin07/12books/pull/54))

**보존 범위는 컬럼마다 다르다** ([#66](https://github.com/irerin07/12books/pull/66))

"행을 남긴다"가 "전부 원형으로 남긴다"는 뜻은 아니다. 목적을 갈라 보면 셋이다.

| 무엇 | 어떻게 | 왜 |
|---|---|---|
| 참조용 `id` | 남긴다 | 글·댓글·좋아요가 가리킨다. 지우면 과거 콘텐츠가 끊긴다 |
| 표시용 `handle`·`display_name` | 남긴다 | 운영 기록에서 "누가 뭘 했나"를 읽어야 한다 |
| 자격증명 `password_hash` | **비운다**(`V15`) | 탈퇴 계정은 로그인도 재발급도 막혀 있어 **할 수 있는 일이 없다.** 보관 기간 내내 원형으로 두면 유출 시 같은 비밀번호를 쓰는 다른 사이트로 넘어간다 |
| `email` | 아직 남긴다 | 재가입 확인·분쟁 대응에 쓸 **여지가 있어** 해시처럼 단정할 수 없다. 다시 볼 시점은 `tasks.md`에 있다 |

상태 전환과 **같은 UPDATE**에서 비운다. 따로 하면 전환에 성공하고 비우기에 실패하는 창이
생기는데, 탈퇴한 계정의 자격증명이 남는 쪽이 더 나쁜 결말이다.

> 그래서 `reissue`가 탈퇴자를 먼저 거르는 것은 취향이 아니라 **필수**다. 지문을 만들 재료가
> 없어 그 자리에서 NPE다.

**하드 삭제를 택했다면 부딪혔을 문제들**(기록으로 남긴다) — `posts.author_id`와
`comments.author_id`가 RESTRICT라 자식부터 지워야 하고, `follows`·`post_likes`는 하드
삭제인데 `posts`·`comments`는 소프트 삭제라 조회 조건을 빼고 지우는 경로가 따로 필요했다.

**남은 것** — 보관 기간이 지난 뒤의 실제 파기(배치·스케줄러가 없다)와 본인인증 도입 시
개인정보 암호화. 둘 다 `tasks.md`의 L2에 조건과 함께 있다.

**완료 기준** — 탈퇴하면 그 사람의 글·댓글이 남의 피드에서 사라지고, 같은 이메일로 다시
가입할 수 있다. **그리고 행은 남아 있다** — 앞만 보면 하드 삭제도 통과한다.

### L2-2. 차단 · Backend

**이 목록에서 가장 큰 작업이다.** 차단은 화면 기능이 아니라 도메인 정책이라,
목록을 만드는 쿼리 전부에 조건이 붙는다.

- 차단 테이블 — `blocks(blocker_id, blocked_id)` + `uk` + 자기 차단 금지 CHECK
- 조건이 붙어야 하는 곳: `findHomePage` · `findTimelinePage` · `findBookPage` ·
  `findAuthorPage` · `findPostPage`(댓글) · `findFollowerPage` · `findFolloweePage`,
  그리고 Phase 7의 사람 검색

> 하나라도 빠뜨리면 "차단했는데 팔로워 목록에선 보인다"가 된다. 소프트 삭제 때
> `deleted_at is null`을 전 쿼리에 손으로 달면서 같은 위험을 겪었고 그때는 하네스를
> 만들지 못했다. **여기서는 만든다** — 목록 쿼리가 차단 조건을 빠뜨리면 빌드가 빨개지는
> 규약 테스트. 빠뜨려도 아무도 모르는 종류의 실수라 사람 눈으로 막을 수 없다.

**완료 기준** — A가 B를 차단하면 피드·책별·사람별·댓글·팔로워/팔로잉·검색 어디에도
서로 보이지 않는다.

### L2-3. 약관 · 개인정보처리방침 · 운영정책 · 연령 · Policy · Frontend

- 이용약관, 개인정보처리방침, 운영정책(욕설·혐오·광고·도배·음란물·저작권·개인정보
  노출·스포일러 처리 기준)
- 가입 화면에서 확인 가능해야 하고, 동의를 법적 근거로 쓰는 항목이 있으면 목적·항목·
  보유기간을 고지한다
- 14세 미만을 받지 않는다면 가입 자격을 명시한다
- 백엔드는 **동의 시각과 약관 버전을 저장**한다 — 나중에 약관이 바뀌면 누가 어느 버전에
  동의했는지가 필요하다

### L2-4. 이메일 인증 · Backend · Frontend

인증이 없으면 남의 이메일로 먼저 계정을 만들 수 있다.

- 가입 → 미인증 → 인증 후 활성
- **미인증 계정이 무엇을 할 수 있는지 정해야 한다.** 읽기만 허용할지, 아무것도 못 하게
  할지. 아무것도 못 하게 하면 가입 이탈이 늘고, 다 허용하면 인증의 의미가 없다
- L1-2의 메일 발송자를 그대로 쓴다

## L3 — 공개와 병행 가능

| 항목 | 구분 | 메모 |
|---|---|---|
| 감상평·댓글 수정 | Backend · Frontend | 지금 **삭제만 있다.** `PATCH /posts/{id}`, `/comments/{id}` |
| 기록하기 플로우 정리 | Frontend | 핵심 행동보다 "닫기"가 더 명확하다는 지적. 작성 중 이탈 확인도 |
| 자체 도메인 · support 주소 | Infra | `*.up.railway.app`은 스테이징 인상을 준다. 메일 발송 도메인과 함께 |
| 카카오 약관 확인 | Policy | 아래 "미해결 전제" 참고 |
| QA 시드 계정 정리 | Infra | 공개 인스턴스에 비밀번호가 같은 계정 15개가 있다 |
| 광고 | — | **출시 후에 붙인다.** 지금 넣으면 동의 관리 요건이 따라와 공개가 늦어진다 |

## 미해결 전제 — 카카오 도서 데이터

리뷰가 지적한 것 중 **혼자 판단할 수 없는 유일한 항목**이다.

확인된 사실부터 적는다.

- REST API 키는 백엔드 환경변수로만 들어가고 카카오 호출은 서버가 한다.
  프런트는 `/api/*`를 백엔드로 프록시할 뿐이라 **키가 번들에 실리지 않는다**
- `books`가 저장하는 것은 `isbn13` · `source_key` · `title` · `authors` · `publisher` ·
  `thumbnail_url` · `published_at`이다. **표지 이미지는 저장하지 않는다** — URL만 두고
  브라우저가 카카오 CDN에서 직접 받는다

남는 질문은 하나다. **검색 결과를 우리 DB에 영구 축적하고 이후 우리 DB에서 제공하는 방식이
약관 범위인가.** 카카오에 문의해 답을 받고 `external-apis.md`에 남긴다. 답에 따라
`books`를 캐시로 다루고 TTL을 두는 구조로 바꿔야 할 수 있다.

## 판본 문제 — 지금은 미루되 여지를 남긴다

같은 책의 다른 판본(출판연도 · 전자책 · 세트)이 각각 독립 `Book`이 되어 감상평이 쪼개진다.
초기에는 ISBN 단위로 간다. 다만 나중에 **작품(work)과 판본(edition)을 나눌 수 있도록**
`books`를 참조하는 쪽이 `book_id` 하나에만 기대게 두고, 판본별 속성을 다른 테이블이
가져갈 여지를 남긴다.
...
| 10 | 알림 (좋아요 · 댓글 · 팔로우) | `V9__notifications` | 반응이 사용자에게 닿는다 |

**출시 준비** (위 "정식 공개 전에 해야 하는 것")

| 묶음 | 내용 | 마이그레이션 |
|---|---|---|
| L1 | 요청 제한 · 비밀번호 재설정 · 신고와 관리자 처리 | 신고 테이블 |
| L2 | 회원탈퇴 · 차단 · 약관과 정책 · 이메일 인증 | 차단 테이블 외 |
| L3 | 감상평 수정 · 기록 플로우 · 도메인 · 카카오 확인 | — |

**출시 준비** (위 "정식 공개 전에 해야 하는 것")

| 묶음 | 내용 | 마이그레이션 |
|---|---|---|
| L1 | 요청 제한 · 비밀번호 재설정 · 신고와 관리자 처리 | `V10__reports` · `V11__admin_hide` |
| L2 | 회원탈퇴 · 차단 · 약관과 정책 · 이메일 인증 | `V13__withdrawal`(완료) · 차단 테이블 |
| L3 | 감상평 수정 · 기록 플로우 · 도메인 · 카카오 확인 | — |
...
| 알림 묶기 ("외 3명") | 읽음 처리가 "어디까지 읽었나"로 복잡해진다. 넣게 되면 행을 합치지 말고 조회 시점에 묶는다 — 합친 행은 되돌릴 수 없다. |
| 알림 실시간 전달 (WebSocket·SSE) | 폴링으로 시작. 실제 요구가 관측된 뒤에. |
| 프로필 동시 수정 방어 (`@Version`) | `@DynamicUpdate`는 **이미 붙어 있다** — 다만 동시 수정 때문이 아니라 프로필 UPDATE가 옛 비밀번호 해시를 되살리는 것을 막으려고 넣었고(`User` javadoc), 그 덕에 서로 다른 필드는 둘 다 남는다. 남은 것은 **같은 필드**를 동시에 고칠 때의 충돌 감지이고, 그건 `@Version` + 409다. 같은 사용자의 드문 상황이라 넣지 않았다. 실제 충돌 사례나 자동 저장 기능이 생길 때. |

spec.md §3.2의 제외 항목(소셜 로그인, 이미지 업로드, 대댓글, DM, 독서 모임,
추천 알고리즘)도 이 계획의 범위 밖이다.

**차단·신고는 예외로 범위에 들어왔다.** UGC 서비스를 불특정 다수에게 여는 조건이라
"정식 공개 전에 해야 하는 것" L1-3·L2-2가 맡는다. 프론트엔드도 별도 리포로 존재한다.
